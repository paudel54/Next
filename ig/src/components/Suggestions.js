            ...
            </div>
            {
                suggestions.map(item => (
                    <div className='flex items-center justify-between mt-3'>
                        <img className='h-10 rounded-full border p-[2px]' src={`https://i.pravatar.cc/150?img=${Math.ceil(Math.random() * 70)}`} alt='Suggesion Areas' />
                        <div className='flex-1 ml-4'>
                            <h2 className='font-semibold text-sm'>{item.username}</h2>
                            <h3 className='font-sm text-gray-400 truncate w-[230px]'>{item.jobTitle}</h3>
                        </div>
